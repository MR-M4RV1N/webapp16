            ...
            @foreach ($catt as $c)
                ...
                @if(isset($cat) && ($cat == $c->id) && !empty($subb[0]))
                        @foreach ($subb as $s)
                            ...
                            @else
                            <p style="font-family:Calibri Light; margin:0px;"><span class=" glyphicon glyphicon-list-alt" aria-hidden="true" style="margin-left:15px; margin-right:5px;"></span><a href="/my_page/{{ $s->route }}/{{ $c->id }}/{{ $s->id }}" style="color: #eeeeee;">{{ $s->name }}</a></p>
                            @endif
                        @endforeach
                        <br>
                    @endif
